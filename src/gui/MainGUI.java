...
/**
 * This class is used for the GUI of the project.
 *
...
public class MainGUI extends JFrame {
     ...
     *
     * Initialises the GUI components, creates the System.out and System.err
     * redirection, checks for file error and corrects any if necessary, creates
     * the user and connector objects as well as updates the comboboxes.
     */
    public MainGUI() {
        initComponents();
        ...
        if (PropertiesFilesUtils.propertiesErrorFound()) {
            ...
            Printer.printErrln("Please re-enter your credentials in the"
                    + " settings menu!");
        } else {
    ...
    /**
     * Updates the database comboboxes according to the current state of the
     * server and then calls the update for the table comboboxes.
     */
    ...
    /**
     * Updates the table comboboxes according to the current database selected
     * and then updates the labels with the correct values.
     */
    ...
    /**
     * Updates the GUI's labels according to the database and table selected.
     */
    ...
    /**
     * Checks for connection to the server and returns the version of it if a
     ...
     *
     * If the database name given is empty, nothing happens.
     *
     * If it is of an already existing database, that database is selected as
     * the current database.
     *
     * Otherwise a new database is created with the name given.
     */
    ...
    /**
     * Deletes the database that has the given name from the server.
     *
     * If the database name given is empty or it is not of an already existing
     * database, nothing happens.
     *
     * Otherwise the database is deleted from the server.
     */
    ...
    /**
     * Creates a table in the currently selected database with the name given.
     *
     * If the table name given is empty, nothing happens.
     *
     * If it is of an already existing table in the currently selected database,
     * that table is selected as the current table.
     *
     * Otherwise a new table is created with the name given in the currently
     * selected database.
     */
    ...
    /**
     * Updates the database comboboxes, table comboboxes and labels.
     */
    ...
    /**
     * Updates the table comboboxes and labels according to the table that was
     * selected.
     *
     * Also sets the current table to the table that was selected or creates a
     * new one if the table doesn't exist and 'Enter' was pressed.
     */
    ...
    /**
     * Tries to find the xampp-control.exe executable in the known paths and
     * then executes it.
     *
     * If it is not found then an error is shown.
     */
    ...
    /**
     * Deletes the table that has the given name from the currently selected
     * database.
     *
     * If the table name given is empty or it is not of an already existing
     * table, nothing happens.
     *
     * Otherwise the table is deleted from the currently selected database.
     */
    ...
    /**
     * Updates the database comboboxes, table comboboxes and labels according to
     * the database that was selected.
     *
     * Also sets the current database to the database that was selected or
     * creates a new one if the database doesn't exist and 'Enter' was pressed.
     */
    ...
    /**
     * Clears the text from jMessageConsoleTextPane.
     */
    ...
    /**
     * Downloads tweets from twitter.com through the twitter API with the
     * Twitter4J library.
     *
     * At first the total runtime calculation occurs from the user input in the
     * appropriate fields. If the total runtime is less than 2 minutes then
     * timeout is set to 0 otherwise timeout is set to 2 minutes. Timeout is
     * used so that the calls in the API do not exceed the Twitter API limit.
     *
     * Then the query is created with the appropriate filters that were used.
     *
     * Now while there is still total runtime left (currentTime - startTime >=
     * totalTime) the program will be running. The maximum number of tweets are
     * downloaded from the API and are added in the _tweets list. Then if there
     * was a city that was specified, the tweets that were not created in that
     * city are deleted from the list. If the "Auto Insert" checkbox was
     * selected, the gathered tweets will be inserted in the database and then
     * deleted from memory otherwise they will remain in memory. In the end the
     * executions waits for the appropriate timeout and then checks if the
     * runtime is over or not.
     *
     */
    ...
    /**
     * When a SinceDate is chosen if the date is equal to the current day the
     * UntilDate has noe meaning and is therefore set to the the current day as
     * well.
     */
    ...
    /**
     * When an UntilDate is chosen the SinceDate is set to no more than the
     * UntilDate.
     */
    ...
    /**
     * Creates a list of String arrays by pulling the field "editedText" from
     * the table currently selected and does the same for the hashtags. It then
     * creates a HTML table which contains two HTML tables, one of the top words
     * and one of the top hashtags properly formatted.
     */
    ...
    /**
     * If a minute, or hour, or day value other than zero is selected then the
     * AutoInsertCheckbox is selected to avoid memory leaks.
     */
